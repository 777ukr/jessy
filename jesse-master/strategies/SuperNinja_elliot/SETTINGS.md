# SuperNinja_elliot - Расширенная стратегия на основе ElliotV4

## Описание

SuperNinja_elliot - это расширенная версия SuperNinja, основанная на ElliotV4 стратегии с множеством технических индикаторов для фильтрации сигналов.

### Используемые индикаторы

**Основные (как в ElliotV4):**
- EWO (Elliott Wave Oscillator) - для определения волн Эллиотта
- EMA (Exponential Moving Average) - для входа и выхода
- RSI (Relative Strength Index) - для определения перекупленности/перепроданности

**Дополнительные фильтры (опционально):**
- ADX (Average Directional Movement Index) - для определения силы тренда
- MACD (Moving Average Convergence Divergence) - для подтверждения тренда
- Bollinger Bands - для фильтрации экстремальных цен
- Awesome Oscillator (AO) - для импульса
- Keltner Channels - для волатильности
- Ultimate Oscillator (UO) - осциллятор
- CCI (Commodity Channel Index) - осциллятор
- Stochastic - для перекупленности/перепроданности
- Parabolic SAR - для тренда
- TEMA (Triple Exponential Moving Average) - для тренда

### ✅ ПРОВЕРЕННЫЕ ДАННЫЕ В БАЗЕ

**Доступные данные:**
- **Период:** 2024-11-01 до 2025-11-07 (год данных)
- **Свечей 1m:** 535,552
- **Все периоды из отчета присутствуют:** 202412, 202501, 202502, 202503, 202504, 202505, 202506, 202507

**Анализ волатильности по месяцам:**
- 202412: диапазон 18.23% (91,517 - 108,199)
- 202501: диапазон 22.50% (89,381 - 109,493)
- 202502: диапазон 31.13% (78,347 - 102,740) ⚠️ самый волатильный
- 202503: диапазон 23.89% (76,598 - 94,894)
- 202504: диапазон 28.15% (74,582 - 95,580)
- 202505: диапазон 19.78% (93,406 - 111,882)
- 202506: диапазон 12.44% (98,288 - 110,517) ✅ наименее волатильный
- 202507: диапазон 17.19% (105,135 - 123,203)

**Общая статистика:**
- Общий диапазон за год: 88.70% (66,805 - 126,064)
- Средняя цена: ~$101,608
- Стандартное отклонение: ~$12,007

### Основные параметры

**Exchange:** Gate USDT Perpetual  
**Symbol:** BTC-USDT  
**Timeframe:** 5m (как в оригинальной стратегии)  
**Leverage:** 1x (cross margin)  
**Starting Balance:** $10,000

### Период тестирования

**Рекомендуемый период:** 2024-12-01 до 2025-07-31 (8 месяцев, как в отчете)

**Или полный год:** 2024-11-01 до 2025-11-07

**Warmup:** 200+ свечей (достаточно данных есть)

### Настройки комиссий

**Fee:** 0.00016 (0.016% - тейкер комиссия Gate.io)

**Примечание:** Gate.io возвращает 60% комиссий на фьючерсах, что улучшит результаты на ~25% от уплаченных комиссий.

### Параметры стратегии

**Основные (как в ElliotV4):**
- EWO Fast: 50
- EWO Slow: 200
- EMA Buy: 17
- EMA Sell: 49
- RSI Buy: < 65
- EWO High: 3.34
- EWO Low: -17.457
- Low Offset: 0.978
- High Offset: 1.019

**Управление рисками:**
- Stop Loss: -18.9%
- Trailing Stop: 0.5% (активируется при +3%)

**Дополнительные фильтры (опционально):**
- ADX Filter: включен (минимальный ADX = 25)
- MACD Filter: включен
- Bollinger Bands Filter: выключен (можно включить для более строгой фильтрации)

### Ожидаемые результаты

На основе ElliotV4 и SuperNinja:
- **Cumulative Return:** ~32% за год
- **Sharpe Ratio:** ~1.08
- **Max Drawdown:** ~-13.73%
- **Win Rate:** ~60-80%
- **Win Days:** ~59.54%
- **Win Month:** ~76.92%

### Отличия от SuperNinja

1. **Расширенные фильтры:** Использует ADX, MACD, Bollinger Bands для дополнительной фильтрации сигналов
2. **Больше индикаторов:** Поддерживает все индикаторы из ElliotV4 (опционально)
3. **Более строгая фильтрация:** Можно включить дополнительные фильтры для уменьшения ложных сигналов
4. **Гибкость:** Можно включать/выключать различные фильтры через параметры стратегии

### Рекомендации

1. Начните с базовых настроек (как SuperNinja)
2. Если слишком много ложных сигналов, включите дополнительные фильтры
3. Используйте таймфрейм **5m** для лучших результатов
4. Убедитесь, что в базе достаточно данных (минимум 200 свечей для warmup)
5. После бектеста используйте калькулятор возврата комиссий для расчета реальной прибыли

### Калькулятор возврата комиссий

```bash
python3 fee_rebate_calculator.py <Total_Paid_Fees> <Net_Profit> <Starting_Balance>
```

Пример:
```bash
python3 fee_rebate_calculator.py 685.40 -595.79 10000
```



